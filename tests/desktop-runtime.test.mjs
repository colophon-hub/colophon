...
import { startLocalColophon } from '../desktop/local-server.mjs'

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const desktopMain = fs.readFileSync(new URL('../desktop/main.mjs', import.meta.url), 'utf8')
function tempRoot() { return fs.mkdtempSync(path.join(os.tmpdir(), 'colophon-desktop-')) }
...
test('desktop local server boots the real API with local owner permission', async () => {
  const root = tempRoot()
  const runtime = await startLocalColophon({ appRoot: repoRoot, dataRoot: root })
  try {
