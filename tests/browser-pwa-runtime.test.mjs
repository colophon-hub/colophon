...
test('browser-local runtime is explicit and separate from desktop and server modes', () => {
  ...
  assert.match(src, /VITE_colophon_RUNTIME/)
})
...
test('PWA has a manifest, offline service worker, install prompt and local media handling', () => {
  ...
  assert.equal(manifest.name, 'Colophon')
...
test('default HTML and PWA manifest do not identify the application as Example Publication', () => {
  const html = read('index.html')
  const manifest = read('public/site.webmanifest')
  assert.doesNotMatch(html, /Example Publication|AberdeenLocal1312|kolektiva\.social/)
  assert.doesNotMatch(manifest, /Example Publication|Exampletown/)
})
