...
test('post editor chrome stays pinned below the fixed desktop admin bar', () => {
  ...
  assert.match(css, /--colophon-editor-sticky-top:\s*32px/)
  assert.match(css, /top:\s*var\(--colophon-editor-sticky-top\)\s*!important/)
...
test('sticky editor offsets account for mobile admin chrome without overlaying it', () => {
  assert.match(css, /@media \(max-width:\s*980px\)[\s\S]*--colophon-editor-sticky-top:\s*78px/)
  assert.match(css, /@media \(max-width:\s*900px\)[\s\S]*--colophon-editor-sticky-top:\s*90px/)
  assert.match(css, /not\(\.admin-frame--rail-collapsed\)[\s\S]*\.admin-rail\s*\{[\s\S]*position:\s*relative\s*!important/)
  assert.match(css, /is-colophon-distraction-free[\s\S]*top:\s*0\s*!important/)
})
...
test('desktop post settings remain sticky and independently scrollable', () => {
  ...
  assert.match(css, /max-height:\s*calc\(100dvh - var\(--colophon-editor-sticky-top\) - 16px\)\s*!important/)
