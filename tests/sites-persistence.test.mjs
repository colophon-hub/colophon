...
test('domain UI and API are provider neutral', () => {
  ...
  assert.match(adapter, /colophon_DEPLOYMENT_PROVIDER/)
  assert.match(adapter, /colophon_DEPLOYMENT_DNS_TARGET/)
  assert.match(api, /isValidHostname/)
  assert.doesNotMatch(component, /colophon\.media|Workers &amp; Pages/)
  assert.doesNotMatch(api, /canonical colophon\.media|CANONICAL_DOMAIN/)
})
