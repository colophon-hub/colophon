...
test('analytics report resolves current native titles by canonical path', () => {
  ...
  assert.equal(resolvePageTitle({ path: '/post/example-story', title: 'Stale Page Title | Example Publication' }, titles), 'Current Editorial Title')
  assert.equal(resolvePageTitle({ path: '/', title: 'Stale Page Title | Example Publication' }, titles), 'Homepage')
