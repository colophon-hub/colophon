const MEDIA_POST_RE = /\/(zine|comic|comics|reader|print|manifesto|colophoneurs)\b|\b(zine|comic|reader|print|manifesto|colophoneurs)\b/i
...
function makeDownloadLinksVisible() {
  document.querySelectorAll('.piece-body__content a[href], .public-experience-sections a[href]').forEach((link) => {
    link.classList.add('colophon-visible-download-link')
...
function apply() {
  if (shouldUseMediaLayout()) {
    document.body.classList.add('is-colophon-preview-media')
    removeDuplicateHeroFromBody()
  } else {
    document.body.classList.remove('is-colophon-preview-media')
  }
