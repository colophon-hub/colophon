const PREVIEW_STORAGE_PREFIX = 'colophon-native-preview-v1:'

function previewSnapshot() {
  if (typeof window === 'undefined') return null
  const params = new URLSearchParams(window.location.search || '')
  const previewId = params.get('preview') || params.get('colophonPreviewPost')
...
function applyPreviewMediaLayout() {
  ...
  document.body.classList.toggle('is-colophon-preview-media', enabled)
  document.body.classList.toggle('is-colophon-preview-article', Boolean(snapshot) && !enabled)
