    ...
    <main className="desktop-welcome desktop-publish-online">
      ...
      <section className="desktop-publish-grid">
        <article className="desktop-publish-card desktop-publish-card--recommended">
          ...
          <p>Creates a portable <code>.colophon</code> backup containing publication setup, posts, collections, publications, podcasts, campaigns, translations, public settings, media metadata and local media files.</p>
          <div className="review-card__actions">
            ...
            <input ref={importRef} hidden type="file" accept=".colophon,application/json,application/vnd.colophon+json" onChange={(event) => importBackup(event.target.files?.[0])} />
          </div>
        ...
        <article className="desktop-publish-card">
          ...
          <p>The tested no-cost route uses a compatible free host and its provided public address. Colophon does not pretend this build can create a cloud account or silently deploy for you.</p>
          <ol>
            <li>Export the publication above.</li>
            <li>Set up a Colophon web instance on a compatible host.</li>
        ...
        <article className="desktop-publish-card">
          ...
          <p>A domain points at a public deployment. It does not replace hosting. Create or choose the public Colophon instance first, then connect the domain.</p>
          {!browserLocal ? <Link className="button" to="/sites">Domain setup</Link> : <span className="description">Domain controls become relevant after you move this publication to a hosted Colophon instance.</span>}
        </article>

        <article className="desktop-publish-card">
          <h2>Community or collective hosting</h2>
          <p>A compatible community host can run the server edition for you. They need the Colophon web app, persistent database storage, persistent media storage, HTTPS and a way to back both stores up.</p>
          <p>This is the closest model to the old bundled Noblogs experience without making Colophon itself a central hosting service.</p>
          <a className="button" href={helpUrl('community-hosting')}>What to ask a host</a>
        </article>

        <article className="desktop-publish-card">
          <h2>Move between browser and desktop</h2>
          <p>The same portable backup format is used by both local editions. Export here, open Colophon desktop, and import the file. No rebuilding the publication by hand.</p>
          {browserLocal ? <a className="button" href="https://github.com/colophon-hub/colophon/releases" target="_blank" rel="noreferrer">Desktop downloads</a> : null}
        </article>
