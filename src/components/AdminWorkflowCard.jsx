...
import { NewsroomPanel } from './NewsroomPanel'
...
function WorkflowStep({ title, body, action, to }) {
  return <article className="admin-workflow-step"><h3>{title}</h3><p>{body}</p><Link className="button button--primary" to={to}>{action}</Link></article>
}
...
export function AdminWorkflowCard() {
  ...
  return <><NewsroomPanel /><section className="admin-workflow-card">
    <EditableText as="div" className="admin-workflow-card__eyebrow" field={block?.eyebrowField || 'admin.workflow.eyebrow'}>editorial workflow</EditableText>
    <EditableText as="h2" field={block?.titleField || 'admin.workflow.title'}>Editorial Workflow</EditableText>
    <EditableText as="div" className="admin-workflow-card__description" field={block?.descriptionField || 'admin.workflow.description'} multiline>Move through archive cleanup, overrides, podcast enrichment, native publishing, and public rendering without relying on memory.</EditableText>
    <div className="admin-workflow-card__grid">
      <WorkflowStep title={title1} body={body1} action={action1} to={adminRoutes.qa} /><WorkflowStep title={title2} body={body2} action={action2} to={adminRoutes.posts} /><WorkflowStep title={title3} body={body3} action={action3} to={adminRoutes.podcasts} /><WorkflowStep title={title4} body={body4} action={action4} to={adminRoutes.nativeBridge} /><WorkflowStep title={title5} body={body5} action={action5} to="/updates" /><WorkflowStep title={title6} body={body6} action={action6} to={adminRoutes.liveEditor} />
    </div>
  </section></>
}
