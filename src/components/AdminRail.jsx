...
const RAIL_STATE_KEY = 'colophon-admin-rail-collapsed-v1'
...
export function AdminRail({ collapsed, onToggleCollapsed }) {
  ...
  const publicationIdentity = effectiveConfig?.identity || {}
  const primarySiteName = String(publicationIdentity.publicationName || 'Independent Publication').trim() || 'Independent Publication'
  const publicationLogo = String(publicationIdentity.logoUrl || '').trim()
  ...
  return (
    <>
      <AdminCommandPalette />
      <div className="wp-admin-topbar" role="navigation" aria-label="Colophon admin bar">
        <div className="wp-admin-topbar__left">
          <Link to={adminRoutes.dashboard} className="wp-admin-topbar__link wp-admin-topbar__link--icon" aria-label="Colophon dashboard" title="Colophon"><span className="wp-admin-topbar__wpicon" aria-hidden="true">S</span></Link>
          ...
          <Link to="/" className="wp-admin-topbar__link wp-admin-topbar__brand-logo-link" aria-label={`${primarySiteName} home`} title={primarySiteName}>
            {publicationLogo ? <img src={publicationLogo} alt={primarySiteName} className="wp-admin-topbar__brand-logo" /> : <span className="wp-admin-topbar__publication-name">{primarySiteName}</span>}
          </Link>
