...
function hasRecentBackup() {
  try {
    const raw = window.localStorage.getItem('colophon-last-backup-at')
    if (!raw) return false
...
export function BrowserBackupReminder() {
  ...
  useEffect(() => {
    ...
    check()
    const refresh = () => check()
    window.addEventListener('colophon:backup-created', refresh)
    window.addEventListener('colophon:portable-imported', refresh)
    return () => { cancelled = true; window.removeEventListener('colophon:backup-created', refresh); window.removeEventListener('colophon:portable-imported', refresh) }
  }, [location.pathname])
