...
export function CampaignPage() {
  ...
  useEffect(() => {
    ...
    setDocumentMeta({
      ...
      image: campaign.heroImage || '/colophon-logo.png',
      type: 'website',
    })
  }, [campaign])
  ...
  return (
    <main className="page campaign-page" data-campaign={campaign.slug}>
      ...
      {showSection('reporting') ? <section className="campaign-section" id="reporting">
        <div className="campaign-shell">
          <SectionHeading sectionKey="reporting" eyebrow="REPORTING + CONTEXT" title={sectionTitle('reporting')} description="The campaign is anchored in reporting and source material. These are the Colophon pieces connected to this campaign." />
...
function SocialSection({ campaign, social, copyState, copyCampaignLink, isAiCampaign, title }) {
  return <section className="campaign-section campaign-section--social" id="social">
    <div className="campaign-shell">
      <SectionHeading sectionKey="social" eyebrow="SOCIAL CIRCULATION" title={title} description={isAiCampaign ? 'Campaign posts from A/I and Colophon. Cavallette is A/I’s official account and posts primarily in Italian; some updates are bilingual.' : 'Public posts and dispatches connected to this campaign.'} />
...
function findCampaignPieces(pieces, campaign) {
  ...
  return pieces.filter((piece) => {
    const explicit = [...(piece.campaigns || []), ...(piece.tags || []), ...(piece.collections || []), ...(piece.projects || []), piece.primaryProject]
      ...
      .some((item) => item === campaignSlug || (isAiCampaign && (item.includes('Example Network') || item.includes('Example Network') || item.includes('a/i campaign'))))
    ...
    return /Example Network(?:\s*\/\s*|\s+)?Example Network/.test(title) || (/communications infrastructure/.test(title) && /terrorism|sanction|designation/.test(title))
