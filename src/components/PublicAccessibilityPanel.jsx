...
const STORAGE_KEY = 'colophon:public-accessibility:v1'
...
function applySettings(settings) {
  ...
  root.dataset.colophonA11yTextSize = settings.textSize
  root.dataset.colophonA11yLineSpacing = settings.lineSpacing
  root.dataset.colophonA11yReadingWidth = settings.readingWidth
  root.dataset.colophonA11yContrast = settings.contrast ? 'on' : 'off'
  root.dataset.colophonA11yLowGlare = settings.lowGlare ? 'on' : 'off'
  root.dataset.colophonA11yPlainReading = settings.plainReading ? 'on' : 'off'
  root.dataset.colophonA11yReducedMotion = settings.reducedMotion ? 'on' : 'off'
}

function syncImageDescriptions(enabled) {
  document.querySelectorAll('[data-colophon-generated-image-description]').forEach((node) => node.remove())
  if (!enabled) return
  ...
  main.querySelectorAll('img[alt]').forEach((image) => {
    ...
    note.dataset.colophonGeneratedImageDescription = 'true'
...
export function PublicAccessibilityPanel() {
  ...
  const clearHighlight = useCallback(() => {
    document.querySelectorAll('[data-colophon-reading-current]').forEach((node) => node.removeAttribute('data-colophon-reading-current'))
  }, [])
  ...
  const highlight = useCallback((index) => {
    ...
    element.setAttribute('data-colophon-reading-current', 'true')
