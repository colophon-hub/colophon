...
export function DesktopWelcomePage() {
  ...
  if (loading) {
    return <main className="desktop-welcome"><section className="desktop-welcome__panel"><p className="desktop-welcome__eyebrow">Colophon</p><h1>Getting things ready…</h1></section></main>
  }
  ...
  return (
    <main className="desktop-welcome">
      ...
      {browserLocal && !showSetup ? (
        <section className="browser-start-grid" aria-label="Ways to use Colophon">
          ...
          <article className="browser-start-card">
            <h2>Install Colophon</h2>
            <p>Install the same local-first web app from your browser. It still stores the publication on this device.</p>
            {isPwaStandalone() ? <span className="description">Colophon is already running as an installed app.</span> : <button className="button" type="button" disabled={!installAvailable} onClick={() => promptPwaInstall()}>{installAvailable ? 'Install app' : 'Install option appears when your browser supports it'}</button>}
          </article>
          <article className="browser-start-card">
            ...
            <a className="button" href="https://github.com/colophon-hub/colophon/releases" target="_blank" rel="noreferrer">Desktop downloads</a>
          </article>
          <article className="browser-start-card">
            <h2>Run Colophon on a server</h2>
      ...
      {showSetup ? (
        <>
          ...
          <p className="desktop-welcome__footnote">Nothing is uploaded by finishing setup. {browserLocal ? 'Clearing this browser’s site data can remove your publication, so Colophon will remind you to export backups.' : 'Your publication stays on this computer until you choose a Publish Online option.'}</p>
        </>
      ) : null}
    </main>
  )
