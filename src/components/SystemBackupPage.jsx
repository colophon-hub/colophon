...
export function SystemBackupPage() {
  const desktop = typeof window !== 'undefined' && Boolean(window.colophonDesktop?.isDesktop)
  ...
  useEffect(() => {
    if (!desktop) return
    window.colophonDesktop.getBackupSettings().then(setDesktopSettings).catch(() => setDesktopSettings(null))
  }, [desktop])

  async function saveDesktopSettings(patch) {
    if (!desktopSettings) return
    const next = await window.colophonDesktop.setBackupSettings({ ...desktopSettings, ...patch })
  ...
  async function handleExport() {
    try {
      ...
      if (!nextSummary.feedSettingsIncluded || !nextSummary.podcastSettingsIncluded || !nextSummary.publicConfigIncluded) {
      ...
      <main className="page wp-admin-screen system-backup-page">
        ...
        {desktop ? (
          <section className="wp-meta-box">
            ...
            <p className="description">Automatic backups are on by default. Colophon checks the schedule while the desktop app is running and keeps rotating local copies of both the SQLite database and media folder.</p>
            {desktopSettings ? (
            ...
            <div className="review-card__actions">
              ...
              <button className="button" type="button" onClick={() => window.colophonDesktop.openBackupFolder()}>Open backup folder</button>
            ...
            <p className="description"><strong>Important:</strong> these copies are on the same computer. For protection against disk loss, theft, or a dead machine, periodically export a portable <code>.colophon</code> backup to another drive or location you control.</p>
          </section>
        ) : (
          <section className="wp-meta-box">
        ...
        <section className="wp-meta-box">
          ...
          <p className="description">Creates a verified data snapshot containing native content and revision history, taxonomy, safe user identity/role metadata, audit events, media metadata, collections, campaigns and revisions, investigations and revisions, publications, Sites &amp; Domains, feed and podcast settings, and public-site configuration.</p>
        ...
        {summary ? (
          <section className="newsroom-stat-grid">
            ...
            <article className="review-summary-card"><div className="review-summary-card__eyebrow">investigations</div><strong>{summary.investigationCount}</strong><span>records</span></article>
            <article className="review-summary-card"><div className="review-summary-card__eyebrow">investigation revisions</div><strong>{summary.investigationRevisionCount}</strong><span>snapshots</span></article>
