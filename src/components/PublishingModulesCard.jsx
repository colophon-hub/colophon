...
export function PublishingModulesCard({ onboarding = false, onComplete }) {
  ...
  async function persist(nextSetup = setup, { completed = false } = {}) {
    setState('saving')
    setNotice('')
    try {
  ...
  function updateIdentity(field, value) {
    setSetup((current) => ({ ...current, identity: { ...current.identity, [field]: value } }))
  }
  ...
  async function uploadLogo(file) {
    ...
    try {
      ...
      setSetup((current) => ({ ...current, identity: { ...current.identity, logoUrl: url } }))
      setLogoState('saved')
      setLogoNotice('Logo uploaded. Finish setup to save it.')
    } catch (error) {
      setLogoState('error')
    ...
    <section className="wp-meta-box publishing-modules-card">
      <div className="wp-screen-header">
        <div>
          ...
          <p className="description">{onboarding ? 'Give the publication a name, then start small. You can change its identity and appearance later in Settings.' : 'Choose which publishing tools this installation uses. Publication identity and appearance live in Public site configuration below.'}</p>
        </div>
      </div>

      {onboarding ? (
        <div className="publishing-modules-card__identity">
          <label><strong>Publication name</strong><input value={setup.identity.name} onChange={(event) => updateIdentity('name', event.target.value)} placeholder="Independent Publication" /></label>
          ...
          <label><strong>Logo URL <small>(optional)</small></strong><input value={setup.identity.logoUrl} onChange={(event) => updateIdentity('logoUrl', event.target.value)} placeholder="https://…" /></label>
          <label><strong>Upload logo <small>(optional)</small></strong><input type="file" accept="image/*" onChange={(event) => uploadLogo(event.target.files?.[0])} disabled={logoState === 'uploading'} /><small>{logoState === 'uploading' ? 'Uploading…' : 'PNG, JPG, WebP, SVG, or another browser-supported image.'}</small></label>
