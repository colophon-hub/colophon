      ...
      <section className="investigation-admin__editor">
        {!draft ? <div className="investigation-admin__empty"><h2>Create the first investigation</h2><button type="button" onClick={startNew}>New investigation</button></div> : <>
          ...
          <ListSection title="Public records desk" items={draft.recordsRequests} onAdd={() => patch({ recordsRequests: [...draft.recordsRequests, normalizeRecordsRequest({ title: '', status: 'Drafting', requestType: 'other' })] })} onRemove={(index) => removeAt('recordsRequests', index)} render={(request, index) => <>
            ...
            <div className="investigation-admin__two"><Field label="Agency subdivision / component"><input value={request.agencyComponent || ''} onChange={(event) => patchList('recordsRequests', index, 'agencyComponent', event.target.value)} /></Field><Field label="Public title"><input value={request.publicTitle || ''} onChange={(event) => patchList('recordsRequests', index, 'publicTitle', event.target.value)} /></Field></div>
            <Field label="Why this request matters"><textarea rows="3" value={request.whyItMatters || ''} onChange={(event) => patchList('recordsRequests', index, 'whyItMatters', event.target.value)} /></Field>
            <Field label="Records sought"><textarea rows="4" value={request.recordsSought || ''} onChange={(event) => patchList('recordsRequests', index, 'recordsSought', event.target.value)} /></Field>
            <Field label="Request text"><textarea rows="6" value={request.requestText || ''} onChange={(event) => patchList('recordsRequests', index, 'requestText', event.target.value)} /></Field>
            <label className="campaign-admin-check"><input type="checkbox" checked={Boolean(request.requestTextPublic)} onChange={(event) => patchList('recordsRequests', index, 'requestTextPublic', event.target.checked)} /> Publish request text</label>
            <label className="campaign-admin-check"><input type="checkbox" checked={request.public !== false} onChange={(event) => patchList('recordsRequests', index, 'public', event.target.checked)} /> Public request</label>
            <div className="investigation-admin__two"><Field label="Date range"><input value={request.dateRange || ''} onChange={(event) => patchList('recordsRequests', index, 'dateRange', event.target.value)} /></Field><Field label="Preferred format"><input value={request.preferredFormat || ''} onChange={(event) => patchList('recordsRequests', index, 'preferredFormat', event.target.value)} /></Field></div>
            <Field label="Fee waiver language"><textarea rows="3" value={request.feeWaiverLanguage || ''} onChange={(event) => patchList('recordsRequests', index, 'feeWaiverLanguage', event.target.value)} /></Field>
            <Field label="Expedited processing language"><textarea rows="3" value={request.expeditedProcessingLanguage || ''} onChange={(event) => patchList('recordsRequests', index, 'expeditedProcessingLanguage', event.target.value)} /></Field>
            ...
            <RecordsDocumentsEditor value={request.documents || []} onChange={(documents) => patchList('recordsRequests', index, 'documents', documents)} />
...
function RecordsDocumentsEditor({ value = [], onChange }) {
  function patch(index, key, nextValue) { onChange(value.map((item, rowIndex) => rowIndex === index ? { ...item, [key]: nextValue } : item)) }
  function add() { onChange([...value, { id: itemId('record-document'), type: 'correspondence', title: '', date: '', url: '', notes: '', public: true }]) }
  function remove(index) { onChange(value.filter((_, rowIndex) => rowIndex !== index)) }
  return <section className="investigation-admin__nested"><div className="investigation-admin__list-actions"><strong>Request documents</strong><button type="button" onClick={add}>Add document</button></div>{!value.length ? <p className="description">No acknowledgements, correspondence, appeals, releases, or responsive records attached yet.</p> : value.map((document, index) => <article className="investigation-admin__row" key={document.id || index}><div className="investigation-admin__row-head"><strong>Document {index + 1}</strong><button type="button" className="is-danger" onClick={() => remove(index)}>Remove</button></div><div className="investigation-admin__two"><Field label="Document type"><select value={document.type || 'other'} onChange={(event) => patch(index, 'type', event.target.value)}>{RECORDS_DOCUMENT_TYPES.map((value) => <option value={value} key={value}>{value}</option>)}</select></Field><Field label="Date"><input type="date" value={document.date || ''} onChange={(event) => patch(index, 'date', event.target.value)} /></Field></div><Field label="Title"><input value={document.title || ''} onChange={(event) => patch(index, 'title', event.target.value)} /></Field><Field label="File / document URL"><input value={document.url || ''} onChange={(event) => patch(index, 'url', event.target.value)} /></Field><Field label="Notes"><textarea rows="2" value={document.notes || ''} onChange={(event) => patch(index, 'notes', event.target.value)} /></Field><label className="campaign-admin-check"><input type="checkbox" checked={document.public !== false} onChange={(event) => patch(index, 'public', event.target.checked)} /> Public document</label></article>)}</section>
}

function FoiaDirectoryLookup(){const [q,setQ]=useState(''),[items,setItems]=useState([]),[notice,setNotice]=useState('');async function search(){setNotice('');try{const r=await fetch(`/api/foia-directory?q=${encodeURIComponent(q)}`,{credentials:'same-origin'}),d=await r.json();if(!r.ok||d.ok===false)throw new Error(d.error||'FOIA.gov lookup failed');setItems(d.components||[])}catch(e){setNotice(e.message)}}return <section className="wp-meta-box"><h2>Optional U.S. FOIA.gov directory</h2><p className="description">Editor-only helper for federal agency components. The public-records model itself remains jurisdiction-agnostic.</p><div className="review-card__actions"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search federal agency components"/><button type="button" className="button" onClick={search}>Search FOIA.gov</button></div>{notice?<p role="status">{notice}</p>:null}{items.slice(0,10).map(i=><p key={i.id}><strong>{i.title}</strong>{i.agencyName?` — ${i.agencyName}`:''}</p>)}</section>}
