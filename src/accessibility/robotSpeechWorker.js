import { rendercolophonSpeech } from './colophonSpeechEngine.js'
...
async function loadSiteVoice() {
  ...
  siteVoicePromise = fetch('/api/public-site-config', {
    ...
    .then(async (res) => {
      ...
      return {
        flite: accessibility.colophonVoice || accessibility.fliteVoice || {},
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    const result = await rendercolophonSpeech(text, speed, options)
    self.postMessage({
      id,
      ok: true,
