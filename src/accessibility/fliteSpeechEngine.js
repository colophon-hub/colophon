export const colophon_FLITE_VOICE = 'cmu_us_lnh'
...
async function fetchBytes(url) {
  ...
  if (!response.ok) throw new Error(`Local Colophon Voice asset failed to load (${response.status}).`)
  return new Uint8Array(await response.arrayBuffer())
}

function runtimeUrl(pathname) {
  if (typeof globalThis.location?.origin !== 'string') throw new Error('Colophon Voice needs a browser origin.')
...
export function decodeWavePcm(bytesLike) {
  ...
  if (bytes.byteLength < 44) throw new Error('Colophon Voice returned an invalid WAV file.')
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength)
  if (readAscii(view, 0, 4) !== 'RIFF' || readAscii(view, 8, 4) !== 'WAVE') {
    throw new Error('Colophon Voice returned an unsupported audio container.')
  }
  ...
  if (!channels || !sampleRate || !dataOffset || !dataSize) throw new Error('Colophon Voice WAV data was incomplete.')
  if (![1, 3].includes(audioFormat)) throw new Error(`Colophon Voice returned unsupported WAV format ${audioFormat}.`)

  const bytesPerSample = Math.ceil(bitsPerSample / 8)
  const frameSize = bytesPerSample * channels
  const frameCount = Math.floor(dataSize / frameSize)
  if (!frameCount) throw new Error('Colophon Voice returned empty audio.')
  ...
  const sampleAt = (sampleOffset) => {
    ...
    throw new Error(`Colophon Voice returned unsupported ${bitsPerSample}-bit WAV audio.`)
  }
...
export async function renderFliteSpeech(text, speed = 1, options = {}) {
  ...
  const outputName = 'colophon-voice.wav'
  const trace = []
  const errors = []
  const root = new PreopenDirectory('.', new Map([
    [`${colophon_FLITE_VOICE}.flitevox`, new File(voiceBytes, { readonly: true })],
  ]))
  const fds = [
  ...
  const args = [
    '--',
    '-psdur',
    '-voice', `${colophon_FLITE_VOICE}.flitevox`,
  ...
  if (exitCode !== 0) {
    throw new Error(`Colophon Voice synthesis failed${errors.length ? `: ${errors.join(' ').trim()}` : ` (exit ${exitCode})`}`)
  }

  const output = root.dir?.contents?.get(outputName)
  const waveBytes = output?.data
  if (!(waveBytes instanceof Uint8Array) || !waveBytes.byteLength) throw new Error('Colophon Voice did not produce audio.')
