...
function boolAttribute(node, name) {
  ...
  return node.hasAttribute(name) ? ` ${name}` : ''
}

function mediaPreload(node) {
  const value = String(node?.getAttribute?.('preload') || '').toLowerCase()
  return ['none', 'metadata', 'auto'].includes(value) ? value : ''
}

export function sanitizeClassicEditorNode(node) {
  if (!node) return ''
  ...
  const children = Array.from(node.childNodes || []).map((child) => sanitizeClassicEditorNode(child)).join('')
  ...
  if (tag === 'audio' || tag === 'video') {
    const src = sanitizeUrl(node.getAttribute('src') || '', { allowFragments: false })
    const preload = mediaPreload(node)
    const poster = tag === 'video'
      ? sanitizeUrl(node.getAttribute('poster') || '', { allowFragments: false })
      : ''
    ...
    const attrs = [
      ...
      boolAttribute(node, 'loop'),
      boolAttribute(node, 'muted'),
  ...
  if (tag === 'source') {
    const src = sanitizeUrl(node.getAttribute('src') || '', { allowFragments: false })
    if (!src) return ''
    const type = String(node.getAttribute('type') || '').trim()
...
function sanitizeHtml(html = '') {
  ...
  return Array.from(doc.body.childNodes || []).map((node) => sanitizeClassicEditorNode(node)).join('')
}
