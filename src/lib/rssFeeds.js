...
function normalizeFeedItem(item, settings) {
  ...
  const author = normalizeFeedTerm('author', item.author || item.byline || 'Colophon Collective', settings) || 'Colophon Collective'
...
export function buildRssBundle(items = [], options = {}) {
  ...
  if (settings.exposeMainFeed !== false) bundle['all-content.xml'] = buildRssXml('Colophon', 'All published Colophon content.', visible, { settings })

  if (settings.exposeProjectFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'projects', titlePrefix: 'Colophon', descriptionPrefix: 'Published content for',
  ...
  if (settings.exposeCollectionFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'collections', titlePrefix: 'Colophon', descriptionPrefix: 'Published content in',
  ...
  if (settings.exposeFormatFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'formats', titlePrefix: 'Colophon', descriptionPrefix: 'Published',
  ...
  if (settings.exposeAuthorFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'bylines', titlePrefix: 'Colophon', descriptionPrefix: 'Published under the public byline label',
    groups: groupBy(visible, 'author', (item) => item.author || item.byline || 'Colophon Collective', settings), settings,
  })
  if (settings.exposeTopicFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'topics', titlePrefix: 'Colophon', descriptionPrefix: 'Published content tagged',
  ...
  if (settings.exposeSeriesFeeds !== false) addGroupedFeeds(bundle, {
    prefix: 'series', titlePrefix: 'Colophon', descriptionPrefix: 'Published content in',
...
export function downloadRssBundle(items = [], options = {}) {
  ...
  anchor.href = url
  anchor.download = `colophon-rss-bundle-${stamp}.json`
