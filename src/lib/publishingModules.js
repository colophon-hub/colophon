const CACHE_KEY = 'colophon-publishing-setup-v1'
const CHANGE_EVENT = 'colophon:publishing-setup'
...
export const DEFAULT_PUBLISHING_SETUP = {
  ...
  identity: { name: 'Independent Publication', description: '', logoUrl: '', primaryEditor: '' },
}
...
function normalize(raw = {}) {
  ...
  return {
    ...
    identity: {
      name: String(raw.identity?.name || DEFAULT_PUBLISHING_SETUP.identity.name).trim() || DEFAULT_PUBLISHING_SETUP.identity.name,
...
async function syncIdentityToPublicConfig(setup) {
  ...
  identity.publicationName = setup.identity.name
  identity.logoUrl = setup.identity.logoUrl
  if (!identity.footerIdentity || identity.footerIdentity === 'Colophon') identity.footerIdentity = setup.identity.name
  if (setup.identity.primaryEditor) identity.correspondenceLabel = setup.identity.primaryEditor

  // Keep these legacy editable fields synchronized while older site templates migrate
  // to the structured identity object. Rendering code should read `identity` directly.
  ...
  const next = { ...current, identity, text }
