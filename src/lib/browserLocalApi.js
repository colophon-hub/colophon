...
const DEFAULT_SETUP = {
  ...
  preset: 'simple',
  modules: ['articles'],
  identity: { name: 'Colophon', description: '', logoUrl: '', primaryEditor: '' },
}
...
async function handleLocalApi(url, input, init) {
  ...
  if (path === '/api/backup-status') return json({ ok: true, mode: 'browser-local', status: 'manual', message: 'Export a portable Colophon backup from Publish Online.' })
  ...
  if (path === '/api/weblate-sync') return json({ ok: false, error: 'Weblate sync requires a server-connected Colophon instance.', mode: 'browser-local' }, 409)
  ...
  return json({ ok: false, mode: 'browser-local', error: 'This tool needs a server-connected Colophon instance. Your local publication data was not uploaded.' }, 501)
}
