const MENUS_KEY = 'colophon-wp-clone-menu-v1'
const SETTINGS_KEY = 'colophon-wp-clone-settings-v1'
const QUICK_DRAFTS_KEY = 'colophon-wp-clone-quick-drafts-v1'
const USER_ROLES_KEY = 'colophon-wp-clone-user-roles-v1'
...
export const DEFAULT_LOCAL_USERS = [
  {
    ...
    email: 'local-editor@colophon.local',
    role: 'Editor',
...
export function loadUserRoleSettings() {
  ...
  const normalized = loaded
    ...
    .map((entry) => ({
      ...
      email: String(entry.email || `${entry.username || entry.id}@colophon.local`),
