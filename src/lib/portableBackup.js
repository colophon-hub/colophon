const FORMAT = 'colophon-portable'
const LEGACY_FORMAT = 'sabotpress-portable'
...
export function validatePortableBackup(bundle) {
  if (!bundle || typeof bundle !== 'object' || Array.isArray(bundle)) throw new Error('Backup file must contain a Colophon JSON object.')
  if (![FORMAT, LEGACY_FORMAT].includes(bundle.format)) throw new Error('This is not a Colophon portable backup.')
  if (Number(bundle.schemaVersion) !== PORTABLE_BACKUP_VERSION) throw new Error(`Unsupported Colophon backup version: ${bundle.schemaVersion}`)
...
export async function downloadPortableBackup() {
  ...
  const blob = new Blob([JSON.stringify(bundle)], { type: 'application/vnd.colophon+json' })
  const href = URL.createObjectURL(blob)
  const anchor = document.createElement('a')
  const date = new Date().toISOString().slice(0, 10)
  anchor.href = href
  anchor.download = `colophon-backup-${date}.colophon`
  document.body.appendChild(anchor)
  anchor.click()
  anchor.remove()
  setTimeout(() => URL.revokeObjectURL(href), 2_000)
  try { window.localStorage.setItem('colophon-last-backup-at', new Date().toISOString()) } catch { /* reminder metadata only */ }
  window.dispatchEvent(new CustomEvent('colophon:backup-created'))
  return bundle
}
...
export async function importPortableBackup(rawBundle) {
  ...
  window.dispatchEvent(new CustomEvent('colophon:portable-imported', { detail: result }))
  return result
}
