...
function contributorSessionHeader(session) { return session ? { 'x-colophon-contributor-session': session } : {} }
...
export async function uploadContributorMedia(file, session, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest(); xhr.open('POST', '/api/campaign-contributor-media'); xhr.setRequestHeader('x-colophon-contributor-session', session); xhr.setRequestHeader('accept', 'application/json')
