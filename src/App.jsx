...
function AnalyticsTracker() {
  ...
  useEffect(() => {
    ...
    const checkMetadata = () => {
      const metadataPath = canonicalizeAnalyticsPath(document.documentElement.dataset.colophonMetaPath || '')
      if (metadataPath !== expectedPath) return
      window.clearTimeout(settleTimer)
      settleTimer = window.setTimeout(() => send(document.documentElement.dataset.colophonMetaTitle || document.title), 250)
    }

    document.addEventListener('colophon:meta-updated', checkMetadata)
    checkMetadata()
    fallbackTimer = window.setTimeout(() => send(''), 4_000)

    return () => {
      document.removeEventListener('colophon:meta-updated', checkMetadata)
      window.clearTimeout(settleTimer)
      window.clearTimeout(fallbackTimer)
    }
  }, [location.pathname])
...
function RouteMeta({ pieces = [] }) {
  ...
  const publicationName = String(resolvedConfig?.identity?.publicationName || 'Independent Publication').trim() || 'Independent Publication'
  ...
  useEffect(() => {
    ...
    if (postMatch) {
      ...
      if (piece) setDocumentMeta(buildPostMeta(piece, { path: pathname, siteName: publicationName }))
      return
    }
    if (postPrintMatch || printMatch) {
      const slug = (postPrintMatch || printMatch)[1]
      const piece = pieces.find((item) => item.slug === slug)
      setDocumentMeta({ ...(piece ? buildPostMeta(piece, { path: pathname, siteName: publicationName }) : {}), title: piece ? `${piece.title} Print` : 'Print', canonicalPath: pathname, siteName: publicationName })
      return
    }
    if (projectMatch) {
      const meta = getProjectMeta(projectMatch[1])
      setDocumentMeta({ title: meta.name, description: meta.description, canonicalPath: pathname, siteName: publicationName })
      return
    }

    const publicRouteMeta = {
      '/': [publicationName, `Latest work from ${publicationName}.`],
      '/archive': ['Archive', `Browse the ${publicationName} archive by search, project, format, and date.`],
      '/collections': ['Collections', `Browse ${publicationName} bodies of work by timeline, downloads, gallery, and related pieces.`],
      '/campaigns': ['Campaigns', 'Campaign hubs gathering reporting, sources, updates, resources, and public materials.'],
      '/feeds': ['Feeds', `Subscribe to ${publicationName} feeds for the archive, formats, projects, collections, and author labels.`],
      '/gallery': ['Gallery', `Browse the ${publicationName} image archive.`],
      '/search': ['Search', `Search the ${publicationName} archive.`],
      '/about': ['About', `About ${publicationName}.`],
      '/contact': ['Contact', `Contact ${publicationName}.`],
      '/submit': ['Submit', `Submit work or information to ${publicationName}.`],
      '/support': ['Support', `Ways to support ${publicationName}.`],
      '/security': ['Security', `Security and source-protection guidance for ${publicationName}.`],
      '/press': ['Press', `Press information and public-facing materials for ${publicationName}.`],
      '/publications': ['Publications', `Read publications and print projects from ${publicationName}.`],
      '/updates': ['Updates', `Latest updates from ${publicationName}.`],
    }[pathname]
    if (publicRouteMeta) {
      setDocumentMeta({ title: publicRouteMeta[0], description: publicRouteMeta[1], canonicalPath: pathname, siteName: publicationName })
      return
    }

    const productRouteMeta = {
      '/login': ['Editor Login', 'Editor login for Colophon administrators.'],
      '/wp-login': ['Editor Login', 'Editor login for Colophon administrators.'],
      '/logout': ['Editor Logout', 'Log out of Colophon editor tools.'],
    }[pathname]
    if (productRouteMeta) setDocumentMeta({ title: productRouteMeta[0], description: productRouteMeta[1], canonicalPath: pathname, siteName: 'Colophon' })
  }, [location.pathname, pieces, publicationName])
  return null
}
...
function ProtectedRoute({ children }) {
  ...
  if (isChecking) {
    return <main className="page admin-login-page"><section className="admin-login-panel"><p className="admin-login-panel__eyebrow">Colophon</p><h1>Checking Access</h1></section></main>
  }
  if (!isAuthenticated) {
    const returnTo = `${location.pathname}${location.search || ''}${location.hash || ''}`
...
function LogoutPage() {
  ...
  return <main className="page admin-login-page"><section className="admin-login-panel"><p className="admin-login-panel__eyebrow">Colophon</p><h1>Logging out</h1><p>Ending your editor session.</p></section></main>
}