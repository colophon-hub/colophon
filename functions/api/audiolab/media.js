...
export async function onRequestPost(context) {
  try {
    ...
    if (!bucket) {
      return json({ ok: false, error: 'Audio storage binding missing. Configure colophon_AUDIO_BUCKET, AUDIO_MEDIA_BUCKET, or MEDIA_BUCKET as an R2 binding.' }, 503)
    }
...
function getAudioBucket(context) {
  return context?.env?.colophon_AUDIO_BUCKET || context?.env?.AUDIO_MEDIA_BUCKET || context?.env?.MEDIA_BUCKET || context?.env?.ASSETS_BUCKET || null
}
