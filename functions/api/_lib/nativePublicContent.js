import { normalizeComments } from './editorialWorkflow.js'
...
function normalizeDisplaySettings(input = {}) {
  ...
  if (settings.defaultMode === 'print' && !settings.enablePrintMode) settings.defaultMode = settings.enableReadMode ? 'read' : settings.enableExperienceMode ? 'experience' : 'read'
  if (settings.defaultMode === 'experience' && !settings.enableExperienceMode) settings.defaultMode = settings.enableReadMode ? 'read' : settings.enablePrintMode ? 'print' : 'read'
  if (settings.defaultMode === 'read' && !settings.enableReadMode) settings.defaultMode = settings.enableExperienceMode ? 'experience' : settings.enablePrintMode ? 'print' : 'read'
  return settings
}

export async function ensureNativePublicContentTable(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS native_public_content (
    id TEXT PRIMARY KEY,
  ...
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_native_public_content_status ON native_public_content(status);').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_native_public_content_target ON native_public_content(target);').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_native_public_content_updated_at ON native_public_content(updated_at DESC);').run()
}

export async function ensureNativeRevisionTable(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS native_public_content_revisions (
    id TEXT PRIMARY KEY,
  ...
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_native_public_content_revisions_native_id ON native_public_content_revisions(native_content_id);').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_native_public_content_revisions_created_at ON native_public_content_revisions(created_at DESC);').run()
}

export function createNativeId() { return `native-${Math.random().toString(36).slice(2, 10)}` }
export function createRevisionId() { return `rev-${Math.random().toString(36).slice(2, 10)}` }
...
export function normalizeNativeEntry(input) {
  ...
  const requestedWorkflowState = normalizeEnum(raw.workflowState || raw.workflow_state, [
    'draft', 'in_review', 'needs_revision', 'ready', 'declined', 'scheduled', 'published', 'archived', 'trash',
  ])
  ...
  return {
    ...
    podcastShowId: String(raw.podcastShowId || ''),
    podcastStorageKey: String(raw.podcastStorageKey || ''),
    podcastMediaId: String(raw.podcastMediaId || ''),
    podcastMimeType: String(raw.podcastMimeType || ''),
    podcastFileSize: Math.max(0, Number(raw.podcastFileSize || 0) || 0),
    ...
    ownerAccountId: String(raw.ownerAccountId || raw.owner_account_id || ''),
    ...
    reviewedBy: String(raw.reviewedBy || raw.reviewed_by || ''),
    declinedAt: normalizeDateString(raw.declinedAt || raw.declined_at || ''),
    declinedBy: String(raw.declinedBy || raw.declined_by || ''),
    editorialComments: normalizeComments(raw.editorialComments || raw.editorial_comments),
...
export async function listNativeEntries(db, options = {}) {
  ...
  const binds = []
  if (options.status) {
    if (options.status === 'published') { clauses.push('(status = ? OR status = ?)'); binds.push('published', 'scheduled') }
    else { clauses.push('status = ?'); binds.push(options.status) }
  }
  if (options.target) { clauses.push('target = ?'); binds.push(options.target) }
  if (options.workflowState) { clauses.push(`json_extract(content_json, '$.workflowState') = ?`); binds.push(options.workflowState) }
  const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : ''
  const stmt = db.prepare(`SELECT id, slug, content_json, status, target, content_type, created_at, updated_at, published_at
    FROM native_public_content ${where} ORDER BY datetime(updated_at) DESC`)
  const result = binds.length ? await stmt.bind(...binds).all() : await stmt.all()
  const rows = Array.isArray(result?.results) ? result.results : []
  const items = normalizeNativeCollection(rows.map(rowToNativeEntry))
...
export async function getNativeEntry(db, idOrSlug, options = {}) {
  await ensureNativePublicContentTable(db)
  const row = await db.prepare(`SELECT id, slug, content_json, status, target, content_type, created_at, updated_at, published_at
    FROM native_public_content WHERE id = ? OR slug = ? LIMIT 1`).bind(idOrSlug, idOrSlug).first()
  if (!row) return null
  const item = normalizeNativeEntry(rowToNativeEntry(row))
  if (!options.includeFuture && !isPubliclyVisible(item)) return null
  return item
}
...
export async function getExistingNativeEntry(db, idOrSlug) {
  ...
  const row = await db.prepare(`SELECT id, slug, content_json, status, target, content_type, created_at, updated_at, published_at
    FROM native_public_content WHERE id = ? OR slug = ? LIMIT 1`).bind(idOrSlug, idOrSlug).first()
  return row ? normalizeNativeEntry(rowToNativeEntry(row)) : null
}

export async function upsertNativeEntry(db, entry) {
  await ensureNativePublicContentTable(db)
  const normalized = normalizeNativeEntry({ ...entry, updatedAt: new Date().toISOString(), publishedAt: computePublishedAt(entry) })
  await db.prepare(`INSERT INTO native_public_content (
      id, slug, content_json, status, target, content_type, created_at, updated_at, published_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(id) DO UPDATE SET
      slug = excluded.slug, content_json = excluded.content_json, status = excluded.status,
      target = excluded.target, content_type = excluded.content_type, updated_at = excluded.updated_at,
      published_at = excluded.published_at`)
    .bind(normalized.id, normalized.slug, JSON.stringify(normalized), normalized.status, normalized.target, normalized.contentType,
      normalized.createdAt, normalized.updatedAt, normalized.publishedAt || null).run()
  return normalized
}

export async function deleteNativeEntry(db, idOrSlug) {
  await ensureNativePublicContentTable(db)
  await db.prepare('DELETE FROM native_public_content WHERE id = ? OR slug = ?').bind(idOrSlug, idOrSlug).run()
...
export async function saveRevisionSnapshot(db, entry, revisionNote = 'autosave') {
  ...
  await db.prepare(`INSERT INTO native_public_content_revisions (
      id, native_content_id, revision_json, revision_note, created_at
    ) VALUES (?, ?, ?, ?, ?)`)
    .bind(createRevisionId(), normalized.id, JSON.stringify(normalized), String(revisionNote || 'autosave'), new Date().toISOString()).run()
  return { ok: true }
}

export async function listRevisionSnapshots(db, nativeId) {
  await ensureNativeRevisionTable(db)
  const result = await db.prepare(`SELECT id, native_content_id, revision_json, revision_note, created_at
    FROM native_public_content_revisions WHERE native_content_id = ? ORDER BY datetime(created_at) DESC`).bind(nativeId).all()
  const rows = Array.isArray(result?.results) ? result.results : []
  return rows.map((row) => ({
    id: row.id,
    ...
    snapshot: normalizeNativeEntry(parseJson(row.revision_json)),
  }))
}

export async function restoreRevisionSnapshot(db, revisionId) {
  await ensureNativeRevisionTable(db)
  const row = await db.prepare(`SELECT id, native_content_id, revision_json, revision_note, created_at
    FROM native_public_content_revisions WHERE id = ? LIMIT 1`).bind(revisionId).first()
  if (!row) throw new Error('revision not found')
  const restored = await upsertNativeEntry(db, { ...parseJson(row.revision_json), updatedAt: new Date().toISOString() })
...
export function isPubliclyVisible(item) {
  ...
  if (['archived', 'trash', 'declined'].includes(String(item.workflowState || ''))) return false
  ...
  return !(scheduled && Number.isFinite(scheduled) && scheduled > Date.now())
}

function rowToNativeEntry(row) {
  const parsed = parseJson(row.content_json)
  return { ...parsed, id: row.id, slug: row.slug, status: row.status, target: row.target, contentType: row.content_type,
...
function normalizeFeaturedTitleDisplay(value) { return normalizeEnum(value, ['overlay', 'below', 'hidden']) }
