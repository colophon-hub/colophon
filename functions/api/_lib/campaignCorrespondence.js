...
export async function contributorFromRequest(db, request) {
  await ensureCampaignCorrespondenceTables(db)
  const dedicated = String(request.headers.get('x-colophon-contributor-session') || '').trim()
