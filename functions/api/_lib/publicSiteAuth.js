...
export async function resolvePublicSitePermission(context) {
  if (isTrustedDesktopRuntime(context)) {
    return bootstrapPermission('desktop-owner', 'desktop-local', 'trusted local Colophon desktop runtime')
  }
  ...
  return deniedPermission(
    ...
    session.reason || (missingSessionSecret(context) ? 'editing locked; set colophon_SESSION_SECRET' : 'valid session required'),
  )
}
...
export function validateAdminLoginToken(context, token) {
  ...
  if (!expected) {
    return { ok: false, reason: 'editing locked; set colophon_ADMIN_TOKEN' }
  }
...
function isTrustedDesktopRuntime(context) {
  if (String(context?.env?.colophon_DESKTOP_LOCAL || '').toLowerCase() !== 'true') return false
  if (String(context?.request?.headers?.get('x-colophon-desktop') || '') !== 'local-runtime') return false
  try {
...
function getSessionTtlSeconds(context) {
  const raw = Number(context?.env?.colophon_SESSION_TTL_SECONDS || 0)
