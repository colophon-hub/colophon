...
export async function fetchBoundedText(rawUrl, options = {}) {
  ...
  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount += 1) {
    ...
    const pending = await timedFetch(current, fetcher, {
      ...
      userAgent: options.userAgent || 'ColophonMediaFeedFetcher/1.0 (+https://example.invalid)',
    })
...
async function resolveHostnameWithDoh(hostname, fetcher) {
  const queries = [1, 28].map(async (type) => {
    ...
    const pending = await timedFetch(url, fetcher, { timeoutMs: 5000, accept: 'application/dns-json', userAgent: 'ColophonMediaDNSGuard/1.0' })
    try {
