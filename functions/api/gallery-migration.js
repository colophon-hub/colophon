...
const MEDIA_BINDING_NAMES = ['colophon_MEDIA_BUCKET', 'MEDIA_BUCKET', 'ASSETS_BUCKET', 'colophon_AUDIO_BUCKET', 'AUDIO_MEDIA_BUCKET']
const LEGACY_HOST = 'colophon-hub.noblogs.org'
...
export async function onRequestPost(context) {
  try {
    ...
    const storage = getMediaBucket(context.env || {})
    if (!storage?.bucket) return json({ ok: false, error: 'R2 media storage binding colophon_MEDIA_BUCKET is required.' }, 503)

    await ensureGalleryTables(db)
    await upsertGallery(db, {
      ...ABERDEEN_1312_GALLERY,
      description: 'Historical Exampletown Local 1312 image archive migrated from the original Colophon Noblogs site.',
    ...
    for (const item of pending) {
      try {
        ...
        const remote = await fetch(source.url, { redirect: 'follow', headers: { 'user-agent': 'Colophon historical media migration' } })
        ...
        const saved = await upsertMediaAsset(db, {
          ...
          description: `Migrated from WordPress attachment ${item.attachmentId} in the Exampletown Local 1312 Gallery.`,
          creator: 'Colophon',
          sourceUrl: source.url,
          folder: 'Images',
    ...
    return json({
      ...
      publicUrl: '/Exampletown-local-1312-gallery/',
...
async function fetchWordPressMedia(attachmentId) {
  try {
    ...
    const response = await fetch(url, { redirect: 'follow', headers: { accept: 'application/json', 'user-agent': 'Colophon historical media migration' } })
...
async function fetchLegacyGalleryImageUrls() {
  const response = await fetch(ABERDEEN_1312_GALLERY.sourceUrl, { redirect: 'follow', headers: { 'user-agent': 'Colophon historical media migration' } })
...
<h1>Exampletown Local 1312 Gallery Migration</h1><p>Source: <a href="${ABERDEEN_1312_GALLERY.sourceUrl}">${ABERDEEN_1312_GALLERY.sourceUrl}</a></p>
...
${complete ? '<p><a class="button" href="/Exampletown-local-1312-gallery/">Open public gallery</a></p>' : ''}
