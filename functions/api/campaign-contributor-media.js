...
const MEDIA_BINDING_NAMES = [CANONICAL_MEDIA_BINDING, 'MEDIA_BUCKET', 'ASSETS_BUCKET', 'colophon_AUDIO_BUCKET', 'AUDIO_MEDIA_BUCKET']
const ALLOWED = new Set([
  'image/jpeg', 'image/png', 'image/webp', 'image/gif', 'image/heic', 'image/heif',
  'audio/webm', 'audio/ogg', 'audio/mpeg', 'audio/mp4', 'audio/x-m4a', 'audio/wav', 'audio/x-wav', 'audio/aac', 'audio/flac',
...
export async function onRequestPost(context) {
  try {
    ...
    if (!storage) {
      return json({
        ok: false,
        error: `Media storage is not configured. Add the ${CANONICAL_MEDIA_BINDING} binding.`,
    ...
    if (!file || typeof file.arrayBuffer !== 'function') return json({ ok: false, error: 'choose an image, audio, video, or PDF file' }, 400)
    ...
    if (!ALLOWED.has(type)) {
      return json({ ok: false, error: `Unsupported media type: ${declaredType || 'unknown'}. Upload a common image, audio, video, or PDF format.` }, 415)
    }

    const bytes = await file.arrayBuffer()
    const validation = validateMediaBytes(bytes, type)
    if (!validation.ok) {
    ...
    const mediaType = type.startsWith('audio/')
      ? 'audio'
    ...
    try {
      await upsertMediaAsset(db, {
        ...
        extension,
        filename,
        folder: 'Campaign Dispatches',
        storageKey: key,
        source: 'campaign-contributor',
        createdAt: new Date().toISOString(),
      })
      await writeAuditLog(db, {
        action: 'campaign_correspondence.media.upload',
        entityType: 'media_asset',
        entityId: mediaId,
        actor: `campaign-contributor:${contributor.id}`,
        detail: {
          campaignId: contributor.campaignId,
          size: file.size,
          mimeType: type,
...
function sanitizeFilename(value) {
  return (String(value || 'dispatch').split(/[\\/]/).pop().trim().replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'dispatch').slice(0, 180)
}
...
function normalizeMimeType(value, filename = '') {
  ...
  return ({
    ...
    heic: 'image/heic', heif: 'image/heif', webm: 'video/webm', mp4: 'video/mp4', mov: 'video/quicktime',
    ogv: 'video/ogg', mp3: 'audio/mpeg', m4a: 'audio/mp4', oga: 'audio/ogg', ogg: 'audio/ogg',
...
function extensionForMime(type, filename = '') {
  ...
  return ({
    ...
    'audio/webm': 'webm', 'audio/ogg': 'ogg', 'audio/mpeg': 'mp3', 'audio/mp4': 'm4a', 'audio/x-m4a': 'm4a',
    'audio/wav': 'wav', 'audio/x-wav': 'wav', 'audio/aac': 'aac', 'audio/flac': 'flac',
    'video/webm': 'webm', 'video/mp4': 'mp4', 'video/quicktime': 'mov', 'video/ogg': 'ogv',
    'application/pdf': 'pdf',
  })[type] || 'media'
}
