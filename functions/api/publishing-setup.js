...
const MODULES = new Set(['articles', 'podcasts', 'campaigns', 'investigations', 'publications', 'translations', 'printlab', 'audiolab'])
const DEFAULT_PUBLICATION_NAME = 'Independent Publication'
const DEFAULT = {
  firstRunComplete: false,
  preset: 'simple',
  modules: ['articles'],
  identity: { name: DEFAULT_PUBLICATION_NAME, description: '', logoUrl: '', primaryEditor: '' },
}
...
function normalize(raw = {}) {
  ...
  const firstRunComplete = Boolean(raw.firstRunComplete)
  const incomingName = String(raw.identity?.name || '').trim()
  const publicationName = !firstRunComplete && (!incomingName || incomingName === 'Colophon')
    ? DEFAULT_PUBLICATION_NAME
    : incomingName || DEFAULT_PUBLICATION_NAME
  return {
    firstRunComplete,
