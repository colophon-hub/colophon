  ...
  try {
    ...
    return json({
      ...
      auth: {
        ...
        adminTokenConfigured: Boolean(context.env?.colophon_ADMIN_TOKEN),
        sessionSecretConfigured: Boolean(context.env?.colophon_SESSION_SECRET),
        cloudflareAccessTrusted: String(context.env?.colophon_TRUST_CF_ACCESS || '').toLowerCase() === 'true',
      },
      bindings: {
        BF_DB: true,
...
function detectMediaBinding(env = {}) {
  for (const name of [CANONICAL_MEDIA_BINDING, 'MEDIA_BUCKET', 'ASSETS_BUCKET', 'colophon_AUDIO_BUCKET', 'AUDIO_MEDIA_BUCKET']) {
