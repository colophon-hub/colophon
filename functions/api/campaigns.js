...
import { deleteCampaign, ensureDefaultCampaigns, getCampaign, listCampaigns, normalizeCampaign, saveCampaignRevision, upsertCampaign } from './_lib/campaigns.js'
...
export async function onRequestGet(context) {
  try {
    ...
    if (slug || id) {
      ...
      let posts = []
      try { posts = await listNativeEntries(db, { status: 'published' }) } catch {}
      return json({ ok: true, mode: 'd1', item: await decorateCampaignAutomation(item, context.request.url, { posts }) })
    }
    return json({ ok: true, mode: 'd1', items: await listCampaigns(db, { includeDrafts }) })
...
export async function onRequestDelete(context) {
  try {
    ...
    await writeAuditLog(db, { action: 'campaigns.delete', entityType: 'campaign', entityId: removed.id, actor: inferActorFromRequest(context.request), detail: { slug: removed.slug, title: removed.title } })
...
async function handleWrite(context) {
  try {
    ...
    await writeAuditLog(db, { action: 'campaigns.upsert', entityType: 'campaign', entityId: saved.id, actor: inferActorFromRequest(context.request), detail: { slug: saved.slug, status: saved.status, lifecycleStatus: saved.lifecycleStatus, updates: saved.updates.length, resources: saved.resources.length } })
