...
export async function onRequestPost(context) {
  try {
    ...
    const bucket = context.env?.colophon_MEDIA_BUCKET || context.env?.MEDIA_BUCKET
    if (!bucket) return json({ ok: false, error: 'colophon_MEDIA_BUCKET is required to preserve Instagram media.' }, 503)
