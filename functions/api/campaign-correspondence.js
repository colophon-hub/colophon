...
import { assignPrivateEditorRecipient, deleteMessageRecipients, listAdminMessagesWithRecipients, listContributorMessages, repairPrivateEditorRecipient } from './_lib/campaignCorrespondencePrivacy.js'
...
export async function onRequestGet(context) {
  try {
    ...
    const campaignView = await publicCampaignWithIdentity(db, campaign)
    const permission = await resolvePublicSitePermission(context)
    const contributor = await contributorFromRequest(db, context.request)
    const contributorSessionSupplied = Boolean(context.request.headers.get('x-colophon-contributor-session') || /^Bearer\s+/i.test(String(context.request.headers.get('authorization') || '')))
    if (contributorSessionSupplied) {
      if (contributor?.campaignId === campaign.id) return json({ ok: true, campaign: campaignView, contributor, messages: await listContributorMessages(db, campaign.id, contributor.id) })
      return json({ ok: false, error: 'Contributor session expired or is invalid. Enter the PIN again.' }, 401)
    }
    if (permission.canEdit && url.searchParams.get('view') === 'admin') return json({ ok: true, campaign: campaignView, contributors: await listContributors(db, campaign.id), messages: await listAdminMessagesWithRecipients(db, campaign.id), questions: await listQuestions(db, campaign.id) })
    return json({ ok: true, campaign: campaignView, messages: await listMessages(db, campaign.id, { publicOnly: true }) })
...
export async function onRequestPost(context) {
  try {
    ...
    if (body.action === 'message' || body.action === 'publish-message') {
      ...
      let recipientContributorId = ''
      if (!actingAsContributor && item.visibility === 'private') {
        try { recipientContributorId = await assignPrivateEditorRecipient(db, campaign.id, item.id, body.recipientContributorId) }
        catch (error) { await deleteMessage(db, item.id, { isEditor: true }); throw error }
      }
      await writeAuditLog(db, { action: 'campaign_correspondence.message.create', entityType: 'campaign_message', entityId: item.id, actor: actingAsContributor ? { type: 'campaign_contributor', id: contributor.id } : inferActorFromRequest(context.request), detail: { campaignId: campaign.id, visibility: item.visibility, directPublicationConfirmed: Boolean(actingAsContributor && publishRequested), recipientContributorId: recipientContributorId || null } })
      const decorated = actingAsContributor ? item : (await listAdminMessagesWithRecipients(db, campaign.id)).find((candidate) => candidate.id === item.id) || item
      return json({ ok: true, item: decorated }, 201)
    }
...
export async function onRequestPatch(context) {
  try {
    ...
    if (body.action === 'recipient') {
      if (!permission.canEdit) return json({ ok: false, error: 'editor access required' }, 403)
      const repaired = await repairPrivateEditorRecipient(db, body.id, body.recipientContributorId)
      await writeAuditLog(db, { action: 'campaign_correspondence.message.recipient', entityType: 'campaign_message', entityId: repaired.messageId, actor: inferActorFromRequest(context.request), detail: { campaignId: repaired.campaignId, recipientContributorId: repaired.contributorId } })
      const item = (await listAdminMessagesWithRecipients(db, repaired.campaignId)).find((candidate) => candidate.id === repaired.messageId)
    ...
    if (body.action === 'message') {
      ...
      await writeAuditLog(db, { action: 'campaign_correspondence.message.update', entityType: 'campaign_message', entityId: item.id, actor: actingAsContributor ? { type: 'campaign_contributor', id: contributor.id } : inferActorFromRequest(context.request), detail: { campaignId: item.campaignId, visibility: item.visibility, contributorMutation: Boolean(actingAsContributor) } })
      const decorated = actingAsContributor ? item : (await listAdminMessagesWithRecipients(db, item.campaignId)).find((candidate) => candidate.id === item.id) || item
      return json({ ok: true, item: decorated })
    }
    return json({ ok: false, error: 'unsupported action' }, 400)
  } catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number(error?.status) || 400) }
}
...
export async function onRequestDelete(context) {
  try {
    ...
    await deleteMessageRecipients(db, item.id)
