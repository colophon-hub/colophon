# Weblate translations in Colophon

Colophon keeps editorial publishing authority in D1 while Weblate provides the collaborative translation workspace.
...
- Renders Colophon-hosted translations on the original article URL with `?lang=<code>`.
...
```json
{
  ...
  "weblateUrl": "https://weblate.example.org/projects/colophon/ai-paranoia/it/",
  "weblateBundle": {
    "title": "…",
    "excerpt": "…",
    "bodyHtml": "…",
...
The endpoint requires Colophon editorial authentication. After review, update `status` to `approved` or `published` with another POST/PUT.
...
Weblate supports standard JSON and nested JSON translation files. Keep the article strings in a normal JSON file rather than inventing a Colophon-specific localization format.

## A/I campaign workflow

1. Export the canonical English A/I article from Colophon.
2. Add that JSON as the English base file in the A/I Weblate component.
3. Give translators the Weblate project/component link instead of Colophon admin access.
4. Translators work and review in Weblate.
5. Import the completed target-language JSON into Colophon with status `in_review`.
...
Existing community translations hosted elsewhere can remain external links. They do not have to be copied into Colophon or falsely presented as Colophon/Weblate translations.

## Database

The schema is defined in `db/native_public_content_translations.sql`. Runtime initialization also lives in `functions/api/_lib/nativePublicTranslations.js`, following the existing Cloudflare D1 initialization pattern used elsewhere in Colophon.
