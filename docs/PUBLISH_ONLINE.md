# Publish Colophon online

You do not need a domain or paid hosting to start using Colophon. The desktop edition keeps the publication on your own computer until you decide to make it public.
...
## $0 route: hosted Colophon on a free tier
...
2. a copy of the Colophon repository in a Git host Cloudflare can deploy from
3. one D1 database bound as `BF_DB`
4. one R2 bucket bound as `colophon_MEDIA_BUCKET`
5. `colophon_ADMIN_TOKEN` and `colophon_SESSION_SECRET` set as secrets
...
In Colophon Desktop open:

**Publish → Publish Online → I already own a domain**

or open **Settings → Domain setup**.

Colophon stores the hostname and shows the DNS target supplied by the deployment. Add that record at the service that manages your DNS. Do not copy DNS values from another person's installation.
...
Colophon itself is free software. A person should be able to:
