...
export async function startLocalColophon({ appRoot, dataRoot, port = 0 }) {
  const databasePath = path.join(dataRoot, 'colophon.sqlite3')
  ...
  const env = {
    BF_DB: db,
    colophon_MEDIA_BUCKET: bucket,
    colophon_DESKTOP_LOCAL: 'true',
    colophon_SESSION_SECRET: sessionSecret,
    colophon_ADMIN_TOKEN: 'desktop-local',
    colophon_DEPLOYMENT_PROVIDER: 'desktop-local',
    colophon_DEPLOYMENT_DNS_TARGET: '',
  }
...
async function handleApi({ req, res, url, appRoot, env }) {
  ...
  headers.delete('x-colophon-desktop')
  headers.set('x-colophon-desktop', 'local-runtime')
...
function serveStatic({ req, res, url, distRoot }) {
  ...
  if (!fs.existsSync(target)) return writeNodeResponse(res, new Response('Colophon build not found', { status: 503 }))
